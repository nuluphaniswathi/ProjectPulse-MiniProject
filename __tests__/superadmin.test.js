...
test("route for getting all projects data by  admin",async()=>{
    ...
    console.log("payload.....",res.body.payload)
    // expect(res.body.payload[0]).toHaveProperty("client");
    // expect(res.body.payload[0]).toHaveProperty("client_account_manager");
    // expect(res.body.payload[0]).toHaveProperty("Domain");
    expect(res.status).toBe(200);
